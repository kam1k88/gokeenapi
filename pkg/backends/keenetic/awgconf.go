...
func (*keeneticAwgconf) AddInterface(confFile string, name string) (models.CreatedInterface, error) {
	b, err := os.ReadFile(confFile)
	if err != nil {
		return models.CreatedInterface{}, err
	}
	if name == "" {
		name = filepath.Base(confFile)
	}

	importData := models.Import{Import: base64.StdEncoding.EncodeToString(b), Name: "", Filename: name}

	var createdInterface models.CreatedInterface
