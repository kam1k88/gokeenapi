...
Use with caution as this bypasses the safety confirmation.`)
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		for _, ifaceId := range interfaces {
			err := keenetic.Checks.CheckInterfaceId(ifaceId)
			if err != nil {
				return err
			}
			err = keenetic.Checks.CheckInterfaceExists(ifaceId)
			if err != nil {
				return err
			}
			routes, err := keenetic.Ip.GetAllUserRoutesRciIpRoute(ifaceId)
			if err != nil {
				return err
			}
		...
		for _, item := range allRoutesToDelete {
			err := keenetic.Ip.DeleteRoutes(item.routes, item.interfaceId)
			if err != nil {
				return err
			}
		}
